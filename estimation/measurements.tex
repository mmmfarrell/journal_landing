...
which has been shown to improve estimates of bias states and constant values~\cite{brink2017partial}. The partial Kalman update provides us a
...
$\hat{\x}$. This correction is done piece-wise where the vector components are
updated with
\begin{equation}
  \hat{\vect{\x}}_{\vect{v}}^{+} = \hat{\vect{\x}}_{\vect{v}} + \tilde{\vect{\x}}_{\vect{v}}
\end{equation}
and the quaternion state is updated with
\begin{equation}
...
% \end{equation}
Using~\eqref{eq:est_paper_Rapprox}, we can expand~\eqref{eq:rft_1} and ignore
