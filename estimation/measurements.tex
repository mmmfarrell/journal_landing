...
non-zero at this point, this estimate is used to correct the estimated state
$\hat{\x}$. This correction is done piece-wise with the vector components
updated as
\begin{equation}
  \hat{\vect{\x}}_{\vect{v}}^{+} = \hat{\vect{\x}}_{\vect{v}} + \tilde{\vect{\x}}_{\vect{v}}
\end{equation}
and the quaternion state updated as
\begin{equation}
...
We assume to have a measurement of the position of the UAV with respect to
the inertial
...
Similar to the position measurement above, we assume to have a measurement of
...
\end{equation}
This can be simplified using~\eqref{eq:quat_true_state} to yield
\begin{align}
  \vect{r}_{\text{att}}
...
\end{align}
where $R_b^c$ and $\vect{p}_{c/b}^b$ are assumed to be known constants.
...
pixel measurements received have already been corrected for lens distortion and
that we known the camera intrinsic matrix,
...
\begin{align}
  ...
  \label{eq:p_i_c_c}
\end{align}
The measurement model and its estimate can, therefore, be written as
\begin{align}
...
\begin{align}
  ...
  R_b^c \hat{R}_I^b \left( \hat{R}_I^g \right)^\transpose .
\end{align}
The derivation of this residual Jacobian can be found in
