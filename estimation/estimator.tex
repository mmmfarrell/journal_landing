...
However, as several of the measurement models depend on the state of the
UAV, we also estimate its state in the same filter to properly
account for the uncertainty in the state. We, therefore, estimate the position,
...
accelerometer and gyroscope sensors used as inputs to the filter.
% to properly
...
target vehicle. We show in the experiments found in~\secref{sec:est_paper_simulation}
