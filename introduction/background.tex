...
these works relied on the detection of infrared LEDs that were
...
motion. For this reason, it is important that a model of the dynamics of the target vehicle
are used by the estimation algorithm to predict the state of the target vehicle
when measurements are not available. Varients of the Kalman filter have been frequently used
...
approaches, however, are likely to fail if the fiducial marker is not detected for
significant periods of time because the target vehicle is likely to not
...
static world.
Visual odometry techniques tracking static features have also been previously applied to landing on
moving platforms~\cite{falanga2017vision}.
During landing, however, the target vehicle occupies progessively more of the
...
vehicle for the duration of the flight.
